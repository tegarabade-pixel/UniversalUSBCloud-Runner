"""Universal USB Cloud v2.12.0 RDP-RECONNECT-CORE native Windows desktop host.
...
class DesktopHost:
    def __init__(self) -> None:
        ...
        self.pending_ice_by_key: dict[tuple[int, int, int], list[dict[str, Any]]] = {}
        ...
        self.retired_sessions: list[int] = []
    ...
    def rtc_config(self, offered_ice: Any = None, offered_provider: str = "", offered_mode: str = "", relay_preference: str = "auto") -> RTCConfiguration:
        ...
        # v2.10.25 RELIABILITY-FIRST: keep the FULL candidate set in AUTO.
        # v2.10.24 filtered host AUTO down to STUN-only, but real diagnostics
        # showed gathering still took ~5s while the remote side lost its relay
        # candidate. When relay<->srflx later became unstable there was no
        # relay<->relay fallback, producing repeated CONNECTED->DISCONNECTED.
        # Full ICE costs the same gather window in practice and restores path
        # redundancy: host + srflx + relay.
    ...
    def _remember_answer(self, key: tuple[int, int, int], payload: dict[str, Any]) -> None:
        ...
        # Retain only the newest few negotiations by insertion order; never infer freshness from numeric session ordering.
    ...
    async def add_remote_ice(self, key: tuple[int, int, int], ice: dict[str, Any]) -> None:
        # ICE candidates belong to an exact negotiation tuple, not merely a media
        # session. This prevents late candidates from revision N being injected into
        # an ICE restart at revision N+1 on the same PeerConnection.
        sid, _, _ = key
        if sid in self.retired_sessions:
            return
        if self.pc is None or key != self.active_offer_key or (sid and sid != self.session_id):
            # A late candidate from an older revision of the same negotiation can never
            # become useful again after an ICE restart. Drop it instead of retaining it.
            if self.active_offer_key is not None:
                a_sid, a_nid, a_rev = self.active_offer_key
                if sid == a_sid and key[1] == a_nid and key[2] < a_rev:
                    return
            queue = self.pending_ice_by_key.setdefault(key, [])
            # Bound candidates per negotiation as well as the number of negotiations.
            # Normal WebRTC gathers only a few dozen; 128 leaves ample headroom while
            # preventing a signaling bug from growing memory without limit.
            if len(queue) < 128:
                queue.append(ice)
            # Keep only a small bounded set of recent negotiation queues.
            while len(self.pending_ice_by_key) > 8:
                oldest = next(iter(self.pending_ice_by_key))
                self.pending_ice_by_key.pop(oldest, None)
            return
    ...
    async def _handle_offer(self, obj: dict[str, Any], offer_generation: int, key: tuple[int, int, int]) -> None:
        ...
        try:
            ...
            reuse_pc = bool(self.pc is not None and sid == self.session_id
                            and getattr(self.pc, "connectionState", "closed") != "closed")
            if reuse_pc:
                pc = self.pc
                LOG.info("RTC same-session renegotiation reusing PeerConnection session=%s negotiation=%s/%s", sid, nid, rev)
            else:
            ...
            if offer_generation != self.offer_generation or self.pc is not pc:
                return
            pending = self.pending_ice_by_key.pop(key, [])
            for ice in pending:
                if offer_generation != self.offer_generation or self.pc is not pc:
                    return
                await self.add_remote_ice(key, ice)
    ...
    def _retire_session(self, sid: int) -> None:
        if sid:
            sid = int(sid)
            if sid not in self.retired_sessions:
                self.retired_sessions.append(sid)
            if len(self.retired_sessions) > 16:
                del self.retired_sessions[:-16]

    async def _start_latest_offer(self, obj: dict[str, Any]) -> None:
        key = self._negotiation_key(obj)
        sid, nid, rev = key
        if sid in self.retired_sessions:
            LOG.info("ignoring retired RTC offer session=%s negotiation=%s/%s", sid, nid, rev)
            return
        if self.session_id and sid and sid != self.session_id:
            self._retire_session(self.session_id)
            LOG.info("retiring previous RTC session=%s for new session=%s", self.session_id, sid)
    ...
    async def handle(self, obj: dict[str, Any]) -> None:
        typ = obj.get("type")
        sid = int(obj.get("sessionId", 0) or 0)
        if sid in self.retired_sessions and typ in {"rtc-offer", "rtc-ice", "rtc-restart", "rtc-answer-replay", "rtc-answer-ack", "rtc-signal-resync"}:
            LOG.info("ignoring retired RTC signal type=%s session=%s", typ, sid)
            return
        if typ == "rtc-offer":
            ...
            if isinstance(ice, dict):
                await self.add_remote_ice(self._negotiation_key(obj), ice)
...
async def main() -> None:
    ...
    async with websockets.serve(host.client, HOST, PORT, max_size=4 * 1024 * 1024, ping_interval=10, ping_timeout=20):
        LOG.info("UUC v2.12.0 RDP-RECONNECT-CORE desktop IPC ready ws://%s:%d max=%dx%d@%dfps touch=%s", HOST, PORT, TARGET_W, TARGET_H, TARGET_FPS, _TOUCH_OK)
