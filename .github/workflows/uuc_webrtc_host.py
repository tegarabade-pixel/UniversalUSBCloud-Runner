"""Universal USB Cloud v2.10.10 MONOTONIC-MEDIA-CLOCK native Windows desktop host.
...
async def main() -> None:
    ...
    async with websockets.serve(host.client, HOST, PORT, max_size=4 * 1024 * 1024, ping_interval=10, ping_timeout=20):
        LOG.info("UUC v2.10.10 MONOTONIC-MEDIA-CLOCK desktop IPC ready ws://%s:%d max=%dx%d@%dfps touch=%s", HOST, PORT, TARGET_W, TARGET_H, TARGET_FPS, _TOUCH_OK)
