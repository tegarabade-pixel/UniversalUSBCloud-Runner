"""Universal USB Cloud v2.10.16 IMMERSIVE-LOCAL-CURSOR native Windows desktop host.
...
class InputInjector:
    def handle(self, obj: dict[str, Any]) -> None:
        ...
        if typ == "mouse-relative":
            ...
            # v2.10.16: reliable button packets may carry the Android local-cursor
    ...
    def _open_dxcam(self, backend: str) -> bool:
        ...
        try:
            ...
            cam = dxcam.create(
                ...
                output_idx=0,
                output_color="BGRA",
                # Keep only the newest couple of compositor frames. A deep ring
...
async def main() -> None:
    ...
    async with websockets.serve(host.client, HOST, PORT, max_size=4 * 1024 * 1024, ping_interval=10, ping_timeout=20):
        LOG.info("UUC v2.10.16 IMMERSIVE-LOCAL-CURSOR desktop IPC ready ws://%s:%d max=%dx%d@%dfps touch=%s", HOST, PORT, TARGET_W, TARGET_H, TARGET_FPS, _TOUCH_OK)
