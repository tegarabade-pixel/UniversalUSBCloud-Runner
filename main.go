...
func (b *Bridge) RunDesktopProxy() {
	go func() {
		for {
			...
			readyPayload, _ := json.Marshal(map[string]any{"type": "desktop-state", "state": "host-ready", "desktopProtocol": 3, "turnMode": os.Getenv("UUC_ICE_MODE"), "iceProvider": os.Getenv("UUC_ICE_PROVIDER")})
			...
			offline, _ := json.Marshal(map[string]any{"type": "desktop-state", "state": "host-offline", "desktopProtocol": 3, "turnMode": os.Getenv("UUC_ICE_MODE"), "iceProvider": os.Getenv("UUC_ICE_PROVIDER")})
