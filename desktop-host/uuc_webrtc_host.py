...
class DesktopHost:
    ...
    def rtc_config(self) -> RTCConfiguration:
        ...
        LOG.info("ICE servers loaded provider=%s mode=%s count=%d stun=%d turn=%d",
                 os.environ.get("UUC_ICE_PROVIDER", "none"),
                 os.environ.get("UUC_ICE_MODE", "direct-only"),
                 len(servers),
    ...
    async def new_pc(self, session_id: int) -> RTCPeerConnection:
        ...
        async def state_change():
            ...
            await self.send({
                ...
                "turnMode": os.environ.get("UUC_ICE_MODE", "direct-only"), "iceProvider": os.environ.get("UUC_ICE_PROVIDER", "none"),
    ...
    async def apply_control(self, obj: dict[str, Any]) -> None:
        ...
        await self.send({
            ...
            "turnMode": os.environ.get("UUC_ICE_MODE", "direct-only"),
            "iceProvider": os.environ.get("UUC_ICE_PROVIDER", "none"),
        })
    ...
    async def handle(self, obj: dict[str, Any]) -> None:
        ...
        if typ == "rtc-offer":
            ...
            await self.send({"type": "rtc-answer", "desktopProtocol": 3, "sessionId": self.session_id, "sdp": pc.localDescription.sdp,
                             "candidateTypes": types, "turnMode": os.environ.get("UUC_ICE_MODE", "direct-only"), "iceProvider": os.environ.get("UUC_ICE_PROVIDER", "none")})
            await self.send({
    ...
    async def client(self, ws) -> None:
        ...
        await self.send({"type": "desktop-state", "state": "host-ready", "desktopProtocol": 3, "sessionId": self.session_id, "turnMode": os.environ.get("UUC_ICE_MODE", "direct-only"), "iceProvider": os.environ.get("UUC_ICE_PROVIDER", "none")})
        try:
            async for raw in ws:
                try:
                    obj = json.loads(raw)
